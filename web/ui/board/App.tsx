...
const TaskRow: FC<{
  ...
  const cyclePriority = () => {
    const i = PRIORITY_ORDER.indexOf(task.priority as (typeof PRIORITY_ORDER)[number]);
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title={`Priority: ${PRIORITY_LABEL[task.priority] ?? "No priority"} — click to change`}
        type="button"
      >
...
export const BoardApp: FC = () => {
  ...
  const { response, isLoading } = list.useGet();
  ...
  const [drag, setDrag] = useState<{ id: string; fromIndex: number; dy: number } | null>(
    null
  );
  ...
  // Keep the rendered order in step with the server, except while a drag
  // is in flight (frozen) or an optimistic reorder is awaiting server
  // confirmation (held until it lands, unless the task set changes).
  useEffect(() => {
  ...
  const handleDragStart = (id: string, e: ReactPointerEvent) => {
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      const target = computeTarget(fromIndex, ev.clientY - startY, rectsRef.current);
  ...
  const styleFor = (index: number): CSSProperties => {
    ...
    let ty = 0;
    if (drag.fromIndex < dragTarget && index > drag.fromIndex && index <= dragTarget)
      ty = -shift;
    else if (dragTarget < drag.fromIndex && index >= dragTarget && index < drag.fromIndex)
      ty = shift;
    ...
    <div className={[css.page, drag ? css.pageDragging : ""].join(" ")}>
      <header className={css.header}>
        ...
        {renaming ? (
          ...
          <h1 className={css.title2} onClick={() => setRenaming(true)} title="Click to rename">
            {name || "Untitled list"}
          </h1>
        )}
